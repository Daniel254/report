...
#            worksheet = workbook.add_sheet(warehouse_id.name)
            ...
            print_date = 'Printed By : '+ str(datetime.date.today().strftime('%d-%m-%Y'))
...
#            
            worksheet[c].write_merge(3, 3, 4,6, 'STOCK INVENTORY',second_header_cus_nubold)
            worksheet[c].write_merge(3, 3, 7,10, 'VALUATED INVENTORY - COST',second_header_cus_nubold)
            worksheet[c].write(4,4, 'Received ',second_header_cus_nubold)
            worksheet[c].write(4,5, 'Sales',second_header_cus_nubold)
            worksheet[c].write(4,6, 'Ending',second_header_cus_nubold)
            worksheet[c].write(4,7, 'C/U',second_header_cus_nubold)
            worksheet[c].write(4,8, 'Received ',second_header_cus_nubold)
            worksheet[c].write(4,9, 'Sales ',second_header_cus_nubold)
            worksheet[c].write(4,10, 'Ending  ',second_header_cus_nubold)
            
            worksheet[c].write(4,0, 'INTERNAL REFERENCE',second_header_cus_nubold)
            worksheet[c].write(4,1, 'PICKING ID',second_header_cus_nubold)
            worksheet[c].write(4,2, 'DATE',second_header_cus_nubold)
            worksheet[c].write(4,3, 'PARTNER',second_header_cus_nubold)
#            
            ...
            for location_id in self.location_id:  
                for product in self.product_ids:
                    ...
                    if move_ids: 
                        ...
                        worksheet[c].write(i,1,'Beginning' ,text_left) 
                        worksheet[c].write(i,2,date ,text_left) 
                        worksheet[c].write(i,3,'Beginning' ,text_left) 
